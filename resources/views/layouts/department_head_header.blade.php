{{--
    Reusable Department Head header
    Usage:
        @include('department_head.partial.header', [
            'title'       => 'My Assets',
            'subtitle'    => 'View and manage your assigned assets',   // optional
            'showSearch'  => true,                                     // optional, default false
            'searchPlaceholder' => 'Search assets...',                 // optional
        ])
--}}
...
@php
    ...
    $searchTarget     = $searchTarget ?? 'auto'; // 'assets' | 'requests' | 'auto'
...
<div class="bg-white border-b border-gray-200 sticky top-0 z-10 shadow-sm">
    <div class="px-4 sm:px-8 py-4 sm:py-5">
        ...
        {{-- Right: search (optional) + action button (optional) + notifications + profile --}}
...
{{-- Notification JS (runs once per page) --}}
<script>
(function () {
    // Prevent double-init if partial is included more than once
    if (window.__dhNotificationsInit) return;
    window.__dhNotificationsInit = true;
...
// ===== Profile dropdown =====
(function () {
    if (window.__dhProfileInit) return;
    window.__dhProfileInit = true;
...
(function () {
    if (window.__dhHeaderSearchInit) return;
    window.__dhHeaderSearchInit = true;
    ...
    // Works for both .filter-btn (assets) and .filter-tab (requests)
    ...
    function passesTabFilter(card, filter) {
        ...
        // Status tabs: Active, For Repair, Pending, Approved, Rejected...
    ...
    function filterList() {
        ...
        // ONLY update the dedicated count element
        const countLabel = document.querySelector('[data-list-count]');
        if (countLabel) {
            const isRequests =
                mode === 'requests' ||
                document.querySelectorAll('.request-row').length > 0;
            countLabel.textContent = isRequests
                ? `${visible} requests`
                : `Showing ${visible} assets`;
        }
    }
    

    // Expose so page scripts can re-run after their own tab clicks
    window.__dhRerunHeaderSearch = filterList;
